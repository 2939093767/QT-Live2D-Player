...
Csm::csmVector<csmString> LAppModel::GetMotionNames()
{
    Csm::csmVector<csmString> result;
    csmMap<csmString, ACubismMotion*>::const_iterator map_ite;

    for (map_ite = _motions.Begin(); map_ite != _motions.End(); map_ite++)
    {
        csmString name = (*map_ite).First;
        result.PushBack(name.GetRawString(),true);
    }
    return result;
}
...
Csm::csmVector<csmString> LAppModel::GetExpressionNames()
{
    Csm::csmVector<csmString> result;
    csmMap<csmString, ACubismMotion*>::const_iterator map_ite;
