...
MyOpenGL::MyOpenGL(QWidget *parent)
{
    ...
    m_toolWidget->setFixedWidth(48);
    m_toolWidget->setWindowFlags(Qt::Tool | Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint);
    m_toolWidget->setAttribute(Qt::WA_TranslucentBackground);
    m_toolWidget->setAutoFillBackground(false);
    m_toolWidget->setStyleSheet("QWidget { background-color: transparent; }");

    // 按钮面板
    m_panelWidget = new QWidget(m_toolWidget);
    ...
    m_toolLayout = new QVBoxLayout(m_toolWidget);
    m_toolLayout->setContentsMargins(0, 0, 0, 0);
    m_toolLayout->setSpacing(6);
    m_toolLayout->addWidget(m_panelWidget);
...
void MyOpenGL::ToolBtnInit(){
    // 示例：添加工具按钮
    addToolButton(":/icons/settings.png", [this]() {
        // 设置按钮点击处理
    });
    addToolButton(":/icons/minimize.png", [this]() {
        setWindowState(Qt::WindowMinimized);
    });
    addToolButton(":/icons/close.png", [this]() {
        close();
    });
}
...
{
    static QVBoxLayout *panelLayout = nullptr;
    if (!panelLayout) {
        panelLayout = new QVBoxLayout(m_panelWidget);
        panelLayout->setContentsMargins(4, 8, 4, 8);
    ...
    btn->setStyleSheet(
        "QPushButton {"
        "   background-color: rgba(70, 70, 75, 200);"
        "   border: none;"
        "   border-radius: 8px;"
        "}"
        "QPushButton:hover {"
        "   background-color: rgba(91, 166, 246, 220);"
        "}"
        "QPushButton:pressed {"
        "   background-color: rgba(74, 150, 245, 255);"
        "}"
    );
...
void MyOpenGL::resizeGL(int w, int h)
{
    ...
    if (m_toolWidget && isVisible()) {
        QPoint globalTopRight = mapToGlobal(QPoint(w - 48, 0));
        m_toolWidget->move(globalTopRight);
    }
}
...
void MyOpenGL::moveEvent(QMoveEvent *event)
{
    ...
    if (m_toolWidget && isVisible()) {
        QPoint globalTopRight = mapToGlobal(QPoint(width() - 48, 0));
        m_toolWidget->move(globalTopRight);
    }
}
