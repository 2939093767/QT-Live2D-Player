...
void LAppView::InitializeSprite()
{
    ...
    // LAppTextureManager::TextureInfo* backgroundTexture = textureManager->CreateTextureFromPngFile(R"(:/image/back_class_normal.png)");
    ...
    // LAppTextureManager::TextureInfo* gearTexture = textureManager->CreateTextureFromPngFile(R"(:/image/icon_gear.png)");
    ...
    // LAppTextureManager::TextureInfo* powerTexture = textureManager->CreateTextureFromPngFile(R"(:/image/close.png)");
...
bool LAppView::OnTouchesIsHit(const Csm::csmChar* area,float px, float py) const
{
    // タッチ終了
    LAppLive2DManager* live2DManager = LAppLive2DManager::GetInstance();
    live2DManager->OnDrag(0.0f, 0.0f);
    {
        ...
        return live2DManager->isHit(area, x, y);
...
void LAppView::SetBackgroundImage(const std::string &imagePath)
{
    LAppTextureManager* textureManager = LAppDelegate::GetInstance()->GetTextureManager();
    if (!textureManager)
    {
    ...
    if (_back)
    {
        GLuint oldTexId = static_cast<GLuint>(_back->GetTextureId());
        textureManager->ReleaseTexture(oldTexId);
        delete _back;
        _back = NULL;
    }

    LAppTextureManager::TextureInfo* backgroundTexture = textureManager->CreateTextureFromPngFile(imagePath);
    ...
    float imgRatio = static_cast<float>(backgroundTexture->width) / static_cast<float>(backgroundTexture->height);
    ...
    _back = new LAppSprite(x, y, fWidth, fHeight, backgroundTexture->id, programId);
    _back->SetWindowSize(width, height);
}
